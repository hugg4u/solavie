...
### 2.8. Đặc Thù Tích Hợp Zalo ZNS & Fallback AWS SES Email

- **Gửi qua Số điện thoại CRM:** ZNS (Zalo Notification Service) cho phép gửi tin nhắn giao dịch (ví dụ: xác nhận lịch hẹn `appointment.confirmed`, nhắc hẹn `appointment.reminder_24h`) trực tiếp tới Số điện thoại (`phone`) của khách hàng được lưu trong hệ thống CRM. Điều này đặc biệt hữu ích khi cuộc trò chuyện đã vượt ngoài cửa sổ 24 giờ của Zalo OA và không thể gửi tin nhắn tự do.
- **Điều kiện ZNS:** Tin nhắn gửi đi phải tuân thủ đúng định dạng Template giao dịch đã được Zalo phê duyệt trước. Số điện thoại của khách hàng bắt buộc phải đang hoạt động trên Zalo.
- **Cơ chế Tự Động Fallback sang AWS SES Email:**
  - Khi hệ thống thực hiện gửi tin nhắn ZNS qua API của Zalo OA, nếu Zalo API trả về lỗi (ví dụ: số điện thoại không sử dụng Zalo, lỗi kết nối Zalo, hoặc hạn mức gửi ZNS bị vượt quá), hệ thống sẽ tự động kích hoạt cơ chế Fallback.
  - Cơ chế Fallback sẽ chuyển đổi nội dung tin nhắn giao dịch đó sang định dạng Email tương ứng và thực hiện gửi đi thông qua dịch vụ **AWS SES (Simple Email Service)** tới Email của khách hàng đã lưu trong CRM.
  - Quá trình fallback phải diễn ra tự động trong cùng một BullMQ Job, ghi nhận chi tiết lỗi của Zalo và cập nhật trạng thái (`FAILED_ZNS_FALLBACK_EMAIL`) vào bảng `notification_logs`.
